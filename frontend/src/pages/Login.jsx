/**
 * Login.jsx — Returning farmer login with mobile + password.
 */
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const data = await login(form.mobile, form.password);
      setAuth(data.farmer_id, data.token, data.is_new_user);
      navigate(data.is_new_user ? '/onboarding/step1' : '/dashboard', { replace: true });
    } catch (err) {
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 pb-10">
        <div className="w-full max-w-sm">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-[#7a90a8] text-sm mt-1">Log in to your KrishiSat account</p>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="pt-1">
                <PrimaryButton
                  ...
                  label="Log In →"
                  loading={loading}
                  disabled={form.mobile.length !== 10 || form.password.length < 6}
                />
              </div>
            </form>
          </div>
          ...
          <p className="text-center text-sm text-[#7a90a8] mt-5">
            ...
            <Link to="/signup" className="text-[#1A6B3C] font-semibold hover:underline">
              Sign Up
            </Link>
          </p>
        </div>
